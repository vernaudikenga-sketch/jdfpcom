...
export default function PublicitesAdminPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/advertising_bookings')
      .then((r) => r.json())
  ...
  return (
    <div className="p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-display text-2xl text-white">Réservations publicitaires</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[hsl(220_16%_12%)] text-[hsl(220_8%_55%)]">
                {['Date', 'Client', 'Société', 'Email', 'Tél.', 'Campagne', 'Début', 'Fin', 'Budget $', 'Statut'].map((h) => (
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.id} className={`border-t border-[hsl(220_12%_20%)] ${i % 2 === 0 ? 'bg-[hsl(220_18%_8%)]' : 'bg-[hsl(220_16%_10%)]'}`}>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-[hsl(220_8%_70%)]">{r.company_name || '—'}</td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-[hsl(220_8%_70%)]">{r.campaign_name || '—'}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[hsl(220_8%_70%)]">{r.start_date ? fmt(r.start_date) : '—'}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[hsl(220_8%_70%)]">{r.end_date ? fmt(r.end_date) : '—'}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[hsl(220_8%_70%)]">
                    {r.budget_usd ? `$${Number(r.budget_usd).toLocaleString()}` : '—'}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <StatusSelect table="advertising_bookings" id={r.id} status={r.status ?? 'pending'} onChange={updateStatus} />
                  </td>
                </tr>
              ))}
            </tbody>

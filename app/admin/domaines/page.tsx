...
export default function DomainesAdminPage() {
  const [rows, setRows] = useState<DomainOrder[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/admin/domain_orders')
      .then((r) => r.json())
  ...
  return (
    <div className="p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-display text-2xl text-white">Commandes domaine / hosting</h1>
      ...
      {loading ? (
        ...
        <p className="text-center py-20 text-[hsl(220_8%_55%)]">Aucune commande pour l&apos;instant.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[hsl(220_16%_12%)] text-[hsl(220_8%_55%)]">
                {['Date', 'Client', 'Email', 'Tél.', 'Produit', 'Domaine', 'Paiement', 'Référence', 'Statut'].map((h) => (
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.id} className={`border-t border-[hsl(220_12%_20%)] ${i % 2 === 0 ? 'bg-[hsl(220_18%_8%)]' : 'bg-[hsl(220_16%_10%)]'}`}>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-[hsl(220_8%_70%)]">{r.product_name || '—'}</td>
                  <td className="px-4 py-3 whitespace-nowrap font-mono text-[hsl(220_8%_70%)]">{r.domain_name || '—'}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[hsl(220_8%_70%)]">{r.payment_method}</td>
                  <td className="px-4 py-3 whitespace-nowrap font-mono text-[hsl(220_8%_70%)]">{r.payment_reference || '—'}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <StatusSelect table="domain_orders" id={r.id} status={r.status ?? 'pending'} onChange={updateStatus} />
                  </td>
                </tr>
              ))}
            </tbody>
